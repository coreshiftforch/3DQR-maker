...
差しこむだけで組め、接着は要りません。受け口は**奥ほど狭いくさび**（すきま 口0.22 → 奥0.10mm）
なので、奥まで押しこむと締まって止まり、カタつきません。**倒れにくさを毎回計算して出します**。
...
- **高さ（厚み）は自動でおさえます**（既定 **0.6mm**）。ロゴは見える面の絵が本体なので、
  Blenderで2mmくらいに作ってあっても<b>高さだけを縮めて</b>入れます（タテヨコはそのまま）。
  0.6mm以下なら本体を厚くせずに収まり、スタンドにも入ります。「モデルのまま」も選べます
- **大きさの微調整**で、読みこんだロゴを **×1.00〜×1.50** まで少しずつ大きくできます
  （厚み＝受け口の深さも同じ倍率で伸び、受け口も一緒に大きくなるのではめ合いは変わりません）
...
**組み立て／分解／印刷の並び**の3つと、**大きさ比較**・**実物大**（それぞれ入／切）。
大きさ比較を押すと、となりに**実物大の10円玉（φ23.5mm）**が出るので、
手に持ったときの大きさがすぐ分かります。分解では**磁石**も見えます。
クリックすると、キーホルダーは「QRを外す→裏返す→もどる」、
スタンドは「QRを抜く→もどる」（スタンドは差し込みが相欠きなので、裏返してははまりません）。

### 実物大で見る（スマホ・PCどちらでも）

**実物大**を押すと、画面の上で**本当の大きさ（1:1）**になります。
刷る前に「思ったより大きい／小さい」が分かるので、イベントで配るときの
大きさ決めがそのまま画面でできます。

- **正射影**（遠近法なし）なので、回しても傾けても**倍率は変わりません**。拡大はできません
  （拡大したら実物大でなくなるため）
- 上に **mmのものさし**が出ます。定規や10円玉を画面に当てれば、その場で確かめられます
- 組み立て／分解／印刷の並びの**どれと組み合わせても**使えます。
  いちばん広い面が正面に来るよう自動で向きが決まり、**正面**でいつでも戻せます
- 画面より大きいときは**ドラッグで動かせます**（スマホでは指1本で移動・2本で回転）
- スマホでは実物大のあいだ**画面ぜんぶをビューに使います**。**もどる**で元に戻ります

#### はじめに一度だけ「画面を合わせる」

ブラウザは「この画面が何インチか」を教えてくれないので、
**1mm が何ピクセルか**を一度だけ合わせます（合わせた値はその端末が覚えます）。

- **方法① カードを当てる**（おすすめ）… クレジットカード・診察券・交通系ICカードは
  どれも **85.6 × 54.0mm**。画面に当てて、枠とぴったり重なるまでつまみを動かす。
  となりの円（10円玉 φ23.5mm）でも確かめられます
- **方法② 定規で測る** … 出てくる線を定規で測って、その長さ（mm）を入れる

合わせる前は **96dpi のつもり**（1mm ≒ 3.78px）で出しています。だいたい合っていますが、
配布物の大きさを決めるときは一度合わせてください。
PCでブラウザの拡大率を変えても、実物大は保たれます。
